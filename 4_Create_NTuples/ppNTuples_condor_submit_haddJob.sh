...
# run my script
Executable     = ppNTuples_condor_run_haddJob.sh
+AccountingGroup = "group_cmshi.ilaflott"
Arguments      = $flavor
#input files. in this case, there are none.
Input          = /dev/null
# log files
Error          = Condor_logs/Condor_haddJob_Flav_${flavor}.err
Output         = Condor_logs/Condor_haddJob_Flav_${flavor}.out
Log            = Condor_logs/Condor_haddJob_Flav_${flavor}.log
